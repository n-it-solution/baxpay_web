    ...
    public function index(Request $request)
    {
        ...
        if($data['password'] == $data['confirmPassword']){
            ...
            if (empty($check)){
                ...
                if (empty($check)) {
                    $sid    = "AC715145e299104c5a601962f13649c43e";
                    $token  = "c58aac58aae067e33e8dbbdf5b0cb472";
                    ...
                    $message = $twilio->messages
                        ->create("+".$data['phone'], // to
                            array(
                                "from" => "BaxPay",
                                "body" => "Your BaxPay Verification code is " . $a
                            )
                        );
                    $user = New User();
                    $user->setCode($a);
...
//                echo date($now);
                    ...
                    $message = (new \Swift_Message('Forget password code'))
                        ->setFrom('maravilhosinga@gmail.com')
                        ->setTo($user->getEmail())
                    ...
                    $message = $twilio->messages
                        ->create("+".$user->getPhoneNumber(), // to
                            array(
                                "from" => "BaxPay",
                                "body" => "Your BaxPay recovery verification code is " . $a
                            )
                        );
        ...
        if ($checkStatus == 0){
            ...
            if($request->get('action') == 'email'){
                ...
                if (!empty($check)) {
                    ...
                    $a = randomPassword();
                    $message = (new \Swift_Message('Welcome to BaxPay'))
                        ->setFrom('maravilhosinga@gmail.com')
                ...
                if (!empty($check)) {
                    ...
                    $message = $twilio->messages
                        ->create("+".$check->getPhoneNumber(), // to
                            array(
                                "from" => "BaxPay",
                                "body" => "Your BaxPay activation code is " . $a
                            )
                        );}
                $check->setCode($a);
                $em->flush();
